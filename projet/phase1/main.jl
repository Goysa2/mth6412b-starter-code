...
nodes_init, edges_init, edges_weight = read_stsp(file)
...
### On ajoute les arêtes à notre graphe avec le poids approprié
for i = 1:length(edges_init)
    for j = i:(length(edges_init)-1)
        poids = edges_weight[i][j-i+1]
        push!(edges_graph, Edge(nodes_graph[i], nodes_graph[j+1], poids))
    end
end
